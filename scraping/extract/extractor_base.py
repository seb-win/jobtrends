...
DETAIL_MAPPING_V01: Dict[str, Any] = {
    "schema_version": "0.1",

    # Welche Input-Arten unterstützt dieser Extractor?
    # "json" oder "html" oder "xml" (xml hier nur, wenn Audi weiterhin XML liefert)
    "supported_input_types": ["json", "html", "xml"],

    # --- JSON Mapping (Keys/Paths) ---
    # Definiere Pfade als Liste von Keys, z.B. ["data","job","responsibilities"]
    # Listen: wenn am Ende eine Liste steht, wird sie als items interpretiert.
    "json": {
        "paths": {
            # Volltext: kann HTML enthalten -> wird bereinigt
            "fulltext": None,  # z.B. ["d","results",0,"JobDescriptionHtml"]

            "overview": None,  # z.B. ["..."]
            "process": None,   # z.B. ["..."]

            "responsibilities_items": None,  # z.B. ["sections","responsibilities","items"]
            "requirements_items": None,
            "additional_items": None,
            "benefits_items": None,
        },

        # Optional: wenn Textfelder HTML enthalten, werden diese als HTML behandelt
        "html_fields": ["fulltext", "overview", "process"],

        # Optional: wenn Items HTML enthalten, werden Items bereinigt
        "html_item_fields": [
    ...
    # --- HTML/XML Mapping (DOM Struktur) ---
    # Alles über CSS-Selektoren + Extraktionsmodus.
    # mode:
    # - "text": Text aus Element
    # - "html": innerHTML -> zu Text bereinigt
    # - "list": list items (li o.ä.) -> List[str]
    "dom": {
        "root_selector": None,  # z.B. "div.job-description" (optional)

        "fields": {
            "fulltext": {"selector": None, "mode": "text"},  # z.B. {"selector":"div.ats-description", "mode":"text"}
        ...
        # Optional: DOM-Elemente entfernen, bevor extrahiert wird
        "remove_selectors": [],
    },
}
