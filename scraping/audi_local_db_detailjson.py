...
from Templates.extractors.audi_detail_extractor import extract_detail_sections
...
from Templates.db_runs import get_conn, touch_jobs_last_seen, mark_jobs_inactive
from Templates.RunMetrics import _update_http_columns_in_db

from Templates.DBNormalize import (
    _prepare_job_for_db,
    _now_utc_iso
)
...
def update_master_list_with_jobs(jobs, master_list):
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            if response:
                ...
                detail_url_abs = None
                if job.get("link"):
                    detail_url_abs = BASE_DETAIL_HOST + job_id_str
    ...
    try:
        from Templates.db_runs import touch_jobs_last_seen
